...
class HomeClass:
    def __init__(self):
        ...
        Button(image = GSButton, command = self.GenSafetyFunc, highlightbackground="black", borderwidth=0).place(x=260, y=600, anchor = CENTER)
        Button(image = PTButton, command = self.PowToolSafetyFunc, highlightbackground="black", borderwidth=0).place(x=500, y=600, anchor = CENTER)
        Button(image = HTButton, command = self.HanToolSafetyFunc, highlightbackground="black", borderwidth=0).place(x=750, y=600, anchor = CENTER)
        Button(image = QTButton, command = self.QuitQuiz, highlightbackground="black", borderwidth=0).place(x=900, y=75, anchor = CENTER)
        window.mainloop()

    def GenSafetyFunc(self):
        canvas1 = Canvas(window, width = 200, height = 50)
        canvas1.place(x=260,y=600,anchor=CENTER)
        canvas1.create_rectangle(0, 0, 200, 50, fill = "black")
        window.iconify()
        GenSafety.GSQuiz()
