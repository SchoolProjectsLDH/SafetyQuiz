...
def ReturnMark(ans):
    ...
    PTRcanvas.create_rectangle(0, 0, 1000, 8000, fill = backgroundcolour)
    PTRcanvas.pack(side = "top", fill = "both", expand = True)
    Label(PTRcanvas,text="Your Result",font=("Helvetica",30),bg=backgroundcolour,fg="white").place(x=500,y=50,anchor=CENTER)
    for iteration in range(0,len(ans)):
        if ans[iteration] == True:
            correct += 1
    mark = (correct/14)*100
    text = str(round(mark,2)) + "%"
    Label(PTRcanvas,text="You got {} out of 14 correct".format(int(correct)),font=("Helvetica",20),bg=backgroundcolour,fg="white").place(x=500,y=200, anchor=CENTER)
    Label(PTRcanvas,text=text,font=("Helvetica",60),fg="white",bg=backgroundcolour).place(x=500,y=350, anchor = CENTER)
